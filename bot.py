...
async def on_ready():
    ...
    try:
        ...
        print(f"⚠️ Failed to fetch/delete global commands: {e}")
    ...
    try:
        ...
        print(f"⚠️ Failed to fetch/delete guild commands: {e}")

    print("✅ All old commands deleted. Registering new ones...")

    # ===== REGISTER AND SYNC NEW COMMANDS =====
    try:
        await bot.tree.sync(guild=guild)
        print(f"✅ Successfully synced NEW commands to guild {GUILD_ID}")
    except Exception as e:
        print(f"⚠️ Failed to sync new commands: {e}")
